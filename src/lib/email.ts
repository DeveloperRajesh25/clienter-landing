...
/**
 * Single From identity for all Clienter mail. Override with EMAIL_FROM
 * (e.g. "Clienter <noreply@clienter.co.in>"). The domain part must be verified
...
async function dispatch(opts: { to: string; subject: string; html: string; text: string }): Promise<void> {
  ...
  if (error) {
    // e.g. "The clienter.co.in domain is not verified" (403), invalid recipient
  ...
  <div style="margin:0;padding:24px 12px;background:${COLOR.canvas};font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid ${COLOR.border};border-radius:18px;overflow:hidden;box-shadow:0 10px 40px -8px rgba(28,25,23,0.12);">
        ...
        <tr><td style="padding:20px 28px;border-bottom:1px solid ${COLOR.border};">
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            <td style="vertical-align:middle;padding-right:10px;">
              <img src="${APP_URL}/logo.png" width="30" height="30" alt="Clienter" style="display:block;border-radius:7px;" />
            </td>
            <td style="vertical-align:middle;">
              <span style="font-size:20px;font-weight:800;letter-spacing:-0.5px;color:${COLOR.textDark};">Clienter</span>
            </td>
          </tr></table>
        </td></tr>
      </table>
      ...
      <div style="padding:20px 28px;border-top:1px solid ${COLOR.border};text-align:center;color:${COLOR.textMuted};font-size:12px;line-height:1.6;">
        <p style="margin:0;">© 2026 Clienter — run your freelance business without the chaos.</p>
        ${waitlistFooter ? `<p style="margin:6px 0 0;">You're receiving this because you joined the Clienter waitlist.</p>` : ''}
      </div>
    </div>
  </div>`
}
...
export async function sendWaitlistConfirmation(email: string, name?: string): Promise<void> {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Thanks for joining the Clienter waitlist — we've saved your spot.</p>
    ...
    <p style="margin:0 0 10px;font-weight:700;color:${COLOR.textDark};">What you'll get with Clienter</p>
  ...
  const text = `You're on the Clienter waitlist!

Hi ${name?.trim() || 'there'},

Thanks for joining the Clienter waitlist — we've saved your spot. As an early member, the first 100 users get 1 month of the Pro plan free.

We'll review your request and email your login details as soon as you're approved (usually 24–48 hours).

What you'll get with Clienter:
...
© 2026 Clienter`
  ...
  await dispatch({
    to: email,
    subject: "You're on the Clienter waitlist 🎉",
...
export async function sendWaitlistApproval(
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Great news — your Clienter waitlist request has been approved and your account is ready.</p>
    ...
    <div style="text-align:center;margin:0 0 4px;">
      ${ctaButton(loginUrl, 'Log in to Clienter →')}
    </div>
  `

  const text = `You're approved! Your Clienter account is ready.
...
© 2026 Clienter`
  ...
  await dispatch({
    to: email,
    subject: "You're approved! Your Clienter account is ready ✅",
    html: shell(body, { waitlistFooter: true }),
    text,
  })
}
...
/**
 * Notify an approved applicant who already had a Clienter account — no
...
export async function sendWaitlistApprovalExisting(email: string, plan: string): Promise<void> {
  ...
  const body = `
    ...
    <p style="margin:0 0 24px;">
      Good news — your Clienter account has been upgraded to the
  ...
  const text = `Your Clienter plan has been upgraded.
...
© 2026 Clienter`
  ...
  await dispatch({
    to: email,
    subject: 'Your Clienter plan has been upgraded ✅',
    html: shell(body),
    text,
  })
}
...
export async function sendTeamMemberCredentials({
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;">Hi ${safeName}, <strong>${safeOrg}</strong> has added you to their workspace on Clienter.</p>
  ...
  const text = `Welcome to ${orgName} on Clienter

Hi ${toName}, ${orgName} has added you to their workspace on Clienter.
...
© 2026 Clienter`
  ...
  await dispatch({
    to: toEmail,
    subject: `You've been added to ${orgName} on Clienter`,
    html: shell(body),
    text,
  })
}
