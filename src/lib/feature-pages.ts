...
const CLIENT_MANAGEMENT: FeaturePageConfig = {
  ...
  intro: {
    heading: 'What is client management in Clienter?',
    body: [
      'Client management is the heart of Clienter: one tidy profile for every client that holds their contact details, company, notes, files, and the complete history of your work together. Instead of scattering that across WhatsApp, email, and spreadsheets, everything about a client lives on a single screen you can pull up in seconds.',
      'Because each profile is connected to the rest of Clienter, opening a client instantly shows their projects, the invoices you’ve raised, the payments you’ve collected, and the meetings you’ve had. When a client messages “where are we at?”, the answer is one click away — not a fifteen-minute hunt through old chats.',
    ],
  },
...
const CRM_PIPELINE: FeaturePageConfig = {
  ...
  intro: {
    ...
    body: [
      'The lead pipeline is Clienter’s visual sales board for the work that hasn’t become a client yet. Every enquiry becomes a card you drag across columns — from first contact to won — so you can see the exact state of your sales at a glance, the same way a Kanban board shows your projects.',
      'You define the stages that match how you sell, with protected Won and Lost columns at the ends. When a lead is won, one click converts it into a full client — carrying the details across so you never re-type anything. When one is lost, Clienter asks for a reason, so you learn what’s actually costing you deals.',
    ],
  },
...
const PROJECT_MANAGEMENT: FeaturePageConfig = {
  ...
  intro: {
    heading: 'What is project management in Clienter?',
    body: [
      'Project management in Clienter turns each piece of client work into a project with its own Kanban board, deadline, budget, files, and tasks. You move work from To-do to In progress to Done by dragging cards, so the status of everything you’re delivering is visible on one board instead of living in your head.',
...
const INVOICING: FeaturePageConfig = {
  ...
  faqs: [
    ...
    { q: 'Is there a free invoice generator?', a: 'Yes — Clienter also offers a free, no-signup invoice generator you can use in your browser, and full invoicing (linked to clients and payments) is built into the app on every plan.' },
  ],
...
const CLIENT_PORTAL: FeaturePageConfig = {
  ...
  keywords: [
    ...
    'client portal on my own domain',
  ],
...
const VERIFIED_REVIEWS: FeaturePageConfig = {
  ...
  h1Highlight: 'reviews',
  subheading:
    'Social proof is the hardest thing for a freelancer to build — so Clienter builds it for you. Real reviews, from real clients, on real completed projects, free on every plan.',
  intro: {
  ...
  faqs: [
    ...
    { q: 'Is it really free?', a: 'Yes. Verified reviews, the public review page, and the embeddable badge are free on every plan, including Free — the more agencies share their badge, the more people discover Clienter.' },
  ],
