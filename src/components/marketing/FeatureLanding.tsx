        ...
        <ul className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2.5 text-sm text-gray-500">
          {TRUST.map(({ icon: TIcon, label }) => (
            <li key={label} className="inline-flex items-center gap-1.5">
              <TIcon className="h-4 w-4 text-orange-500" />
              {label}
            </li>
          ))}
        </ul>
            ...
            {steps.map((step, i) => (
              <Reveal key={step.title} delay={i * 80}>
                <div className="flex gap-4 rounded-2xl border border-stone-200/70 bg-white/60 p-6 shadow-soft backdrop-blur-sm">
                  <span className="flex h-11 w-11 flex-none items-center justify-center rounded-2xl bg-gradient-to-b from-orange-500 to-orange-600 font-display text-lg font-bold text-white shadow-lg shadow-orange-500/30">
                    {i + 1}
                  </span>
                  <div>
          ...
          <div className="mt-14 grid gap-x-10 gap-y-10 sm:grid-cols-2">
            {helps.map(({ icon: HIcon, title, desc }, i) => (
              <Reveal key={title} delay={i * 90} className="group flex gap-4">
                <span className="flex h-11 w-11 flex-none items-center justify-center rounded-xl bg-gradient-to-br from-orange-500 to-amber-500 text-white shadow-lg shadow-orange-500/25 transition-transform duration-300 group-hover:-translate-y-0.5">
