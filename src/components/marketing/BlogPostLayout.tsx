  ...
  return (
    <PageShell>
      ...
      <header className="relative overflow-hidden pt-28 sm:pt-36">
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-sm text-gray-500">
            ...
            <Link href={`/blog/category/${post.categorySlug}`} className="hover:text-orange-600">{post.category}</Link>
          </nav>
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {toc.length > 2 && (
          <nav className="mb-10 rounded-2xl border border-stone-200/70 bg-white/60 p-5 shadow-soft backdrop-blur-sm">
            ...
            <ul className="mt-3 space-y-2">
              {toc.map((h) => (
                <li key={h.id}>
                  <a href={`#${h.id}`} className="text-[15px] text-gray-600 transition-colors hover:text-orange-600">
                    {h.text}
                  </a>
                </li>
              ))}
        ...
        {/* Inline CTA */}
        <div className="my-12 rounded-3xl border border-orange-200 bg-gradient-to-br from-orange-50 to-amber-50 p-7 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-[15px] text-gray-600">
            Clienter brings clients, projects, invoices, and payments together — free to start.
          </p>
          <Link
            href="/pricing"
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-stone-200/70 bg-white/60 p-5">
          <div className="flex h-12 w-12 flex-none items-center justify-center rounded-full bg-gradient-to-br from-orange-500 to-amber-500 font-display text-lg font-bold text-white">
        ...
        {post.related.length > 0 && (
          <section className="mt-14">
            <h2 className="font-display text-sm font-bold uppercase tracking-[0.16em] text-stone-400">Related in Clienter</h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              {post.related.map((r) => (
                <Link
                  key={r.href}
                  href={r.href}
                  className="group rounded-xl border border-stone-200/70 bg-white/60 p-4 transition-all hover:-translate-y-0.5 hover:border-orange-200"
                >
                  <span className="font-display text-sm font-bold text-gray-900 group-hover:text-orange-600">{r.label}</span>
