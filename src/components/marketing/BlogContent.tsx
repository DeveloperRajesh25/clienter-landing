    ...
    <div className="space-y-6">
      {blocks.map((block, i) => {
        switch (block.type) {
              ...
              <ul key={i} className="space-y-2.5 pl-1">
                {block.items.map((item, j) => (
                  <li key={j} className="flex gap-3 text-lg leading-relaxed text-gray-700">
                    <span className="mt-2.5 h-1.5 w-1.5 flex-none rounded-full bg-orange-400" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
          ...
          case 'ol':
            return (
              <ol key={i} className="space-y-2.5">
                {block.items.map((item, j) => (
                  <li key={j} className="flex gap-3 text-lg leading-relaxed text-gray-700">
                    <span className="flex h-6 w-6 flex-none items-center justify-center rounded-full bg-orange-100 font-display text-xs font-bold text-orange-600">
                      {j + 1}
                    </span>
                    <span>{item}</span>
                  </li>
          ...
          case 'callout':
            return (
              <div key={i} className="flex gap-3 rounded-2xl border border-orange-200 bg-orange-50/50 p-5">
                <Info className="mt-0.5 h-5 w-5 flex-none text-orange-500" />
