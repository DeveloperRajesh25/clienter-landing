...
export function GlossaryTermPage({ config }: { config: GlossaryTermConfig }) {
  const { term, path, category, definition, body, clienterNote, related, relatedTerms } = config
          ...
          {clienterNote && (
            <Reveal className="mt-10">
              <div className="rounded-2xl border border-orange-200 bg-orange-50/50 p-6">
                <h2 className="font-display text-lg font-bold text-gray-900">How it works in Clienter</h2>
                <p className="mt-2 leading-relaxed text-gray-700">{clienterNote}</p>
                <Link
                  href="/features"
                  className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-orange-600 hover:text-orange-700"
                >
                  Explore Clienter’s features
              ...
              <div className="mt-4 flex flex-wrap gap-2.5">
                {relatedTerms.map((t) => (
                  <Link
                    ...
                    className="rounded-full border border-stone-200 bg-white/70 px-4 py-1.5 text-sm font-medium text-gray-700 transition-colors hover:border-orange-200 hover:text-orange-600"
                  >
                    {t.term}
                  </Link>
                ))}
