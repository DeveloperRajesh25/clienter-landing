...
export function SpotlightButton({
  ...
  return (
    <a
      ref={ref}
      href={href}
      className={`press group relative inline-flex items-center justify-center overflow-hidden rounded-full bg-gray-900 text-white shadow-[0_12px_34px_-12px_rgba(0,0,0,0.6)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_44px_-12px_rgba(249,115,22,0.55)] ${className}`}
    >
      {/* Liquid water drop — solid, outlined circle (no highlight dot) */}
      <span
        ref={dropRef}
        aria-hidden
        className={`pointer-events-none absolute left-0 top-0 rounded-full bg-orange-500 ring-2 ring-orange-200/80 shadow-[inset_0_-3px_6px_rgba(154,52,18,0.45),0_6px_16px_rgba(249,115,22,0.55)] will-change-transform ${dropClassName}`}
